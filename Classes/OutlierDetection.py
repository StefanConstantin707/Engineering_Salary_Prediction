...
class OutlierDetector(BaseEstimator, TransformerMixin):
    """
    Detects and removes outliers from the training data using various methods.
    During transform, it only removes outliers if in training mode.
    """
    ...
    def fit_transform(self, X, y=None):
        """
        Fit and transform in one step, properly handling y.
        """
        self.fit(X, y)
...
class OutlierRemovalWrapper(BaseEstimator, TransformerMixin):
    """
    Wrapper that handles outlier removal for pipelines.
    Ensures that y is also filtered when outliers are removed.
    """

    def __init__(self, outlier_detector, return_indices=False):
        self.outlier_detector = outlier_detector
        self.return_indices = return_indices
        self.inlier_indices_ = None

    def fit(self, X, y=None):
        """Fit the outlier detector."""
        self.outlier_detector.fit(X, y)
        self.inlier_indices_ = ~self.outlier_detector.outlier_mask_
        return self

    def transform(self, X):
        """Transform X, removing outliers only during training."""
        # During prediction, return all data
        if not hasattr(self, 'training_mode_') or not self.training_mode_:
            return X

        # During training, remove outliers
        if hasattr(X, 'iloc'):
            return X.iloc[self.inlier_indices_].copy()
        else:
            return X[self.inlier_indices_]

    def fit_transform(self, X, y=None):
        """Fit and transform, with special handling for training mode."""
        self.training_mode_ = y is not None
        self.fit(X, y)
        return self.transform(X)

    def get_inlier_indices(self):
        """Get boolean mask of inliers."""
        return self.inlier_indices_
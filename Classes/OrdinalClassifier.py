...
from sklearn.multiclass import _fit_binary as _fit_ovr_binary
        ...
        if self.classes_.shape[0] > 2:
            ...
            self.estimators_ = Parallel(n_jobs=self.n_jobs)(
                delayed(_fit_ovr_binary)(
                    clone(self.estimator),  # a fresh clone per job
                    X,  # training features
                    y_d,  # one column of derived_y
                    classes=[  # two-class labels for this binary problem
                        f"not {self.classes_[i]}",
                        " or ".join(str(c) for c in self.classes_[i + 1:])
                    ],
                    fit_params={}  # empty fit_params dict
                )

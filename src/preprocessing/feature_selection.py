...
class FeatureSelector(BaseEstimator, TransformerMixin):
    """
    Comprehensive feature selection transformer supporting multiple methods.
    ...
    def _fit_chi2(self, X, y, n_features):
        ...
        selector = SelectKBest(chi2, k=int(n_features))
    ...
    def _fit_tree_importance(self, X, y, n_features):
        ...
        indices = np.argsort(importances)[::-1][:int(n_features)]
    ...
    def _fit_lasso(self, X, y, n_features):
        ...
        selector = SelectFromModel(lasso, max_features=int(n_features))
        selector.fit(X_array, y)
    ...
    def _fit_rfe(self, X, y, n_features):
        ...
        selector = RFE(estimator, n_features_to_select=int(n_features), step=0.1)
        selector.fit(X_array, y)
    ...
    def _fit_ensemble(self, X, y, n_features):
        ...
        if len(selected) < int(n_features):
            remaining = sorted(
                [(f, v) for f, v in feature_votes.items() if f not in selected],
                key=lambda x: x[1],
                reverse=True
            )
            selected.extend([f for f, v in remaining[:int(n_features) - len(selected)]])

        self.selected_features_ = selected[:int(n_features)]
...
def analyze_feature_importance(pipeline, X, y, top_n=20):
    ...
    pipeline : sklearn.pipeline.Pipeline
        Fitted pipeline containing feature selection
    X : array-like
        Input features
    y : array-like
        Target values
    ...
    preproc_pipeline = SKPipeline(preproc_steps)
